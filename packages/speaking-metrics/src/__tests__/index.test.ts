...
describe('mapSpeakingScoreToCoaching', () => {
  ...
  it('handles prosody goal with flat pitch', () => {
    ...
    expect(result.actionTip).toBe(
      'Exaggerate the pitch on every important word — go over the top on purpose.'
    );
  });
  ...
  it('handles static goals: resonance, articulation, breath_support', () => {
    ...
    expect(resonanceResult.correctionMessage).toBe(
    ...
    expect(articulationResult.correctionMessage).toBe(
    ...
    expect(breathResult.correctionMessage).toBe(
