...
describe('mapSpeakingScoreToCoaching', () => {
  ...
  it('maps too fast pace correctly', () => {
    const result = mapSpeakingScoreToCoaching({ ...defaultScore, pace: 60, overall: 60 }, 'pace', {
      ...defaultAnalysis,
      wpm: 170,
    });
  ...
  it('maps prosody uptalk correctly', () => {
    const result = mapSpeakingScoreToCoaching(defaultScore, 'prosody', {
      ...defaultAnalysis,
      uptalkRatio: 0.5,
    });
    expect(result.correctionMessage).toBe(
  ...
  it('maps filler_reduction high correctly', () => {
    const result = mapSpeakingScoreToCoaching(defaultScore, 'filler_reduction', {
      ...defaultAnalysis,
      fillerRate: 5,
    });
  ...
  it('maps authority uptalk correctly', () => {
    const result = mapSpeakingScoreToCoaching(defaultScore, 'authority', {
      ...defaultAnalysis,
      uptalkRatio: 0.6,
    });
  ...
  it('maps other goals correctly', () => {
    expect(
      mapSpeakingScoreToCoaching(defaultScore, 'resonance', defaultAnalysis).correctionMessage
    ).toContain('vibration in your chest');
    expect(
      mapSpeakingScoreToCoaching(defaultScore, 'articulation', defaultAnalysis).correctionMessage
    ).toContain('Consonants were clear');
    expect(
      mapSpeakingScoreToCoaching(defaultScore, 'breath_support', defaultAnalysis).correctionMessage
    ).toContain('flowing through to the end');
  });

  it('maps success bands correctly based on overall score', () => {
    const excellent = mapSpeakingScoreToCoaching(
      { ...defaultScore, overall: 90 },
      'pace',
      defaultAnalysis
    );
    ...
    const good = mapSpeakingScoreToCoaching(
      { ...defaultScore, overall: 75 },
      'pace',
      defaultAnalysis
    );
    ...
    const developing = mapSpeakingScoreToCoaching(
      { ...defaultScore, overall: 60 },
      'pace',
      defaultAnalysis
    );
    ...
    const retry = mapSpeakingScoreToCoaching(
      { ...defaultScore, overall: 40 },
      'pace',
      defaultAnalysis
    );
