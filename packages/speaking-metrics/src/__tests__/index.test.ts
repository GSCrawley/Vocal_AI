...
describe('generateSpeakingFeedback', () => {
  ...
  it('returns default feedback for unknown failureMode', () => {
    const feedback = generateSpeakingFeedback('unknown_mode');
    expect(feedback).toEqual({ text: 'Focus on your rhythm and pace.' });
  });
});
